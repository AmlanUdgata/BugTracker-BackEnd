...
public class AdminController {
	...
	@PostMapping("add")
	public ResponseEntity<?> createNewAdmin(@Valid @RequestBody Admin bugEntity,BindingResult result) {
		ResponseEntity<?> errorMap=errorMapValidationService.mapValidationError(result);
		if(errorMap!=null) return errorMap;
		Admin savedAdmin = adminService.saveOrUpdate(bugEntity);
		return new ResponseEntity<>(savedAdmin,HttpStatus.CREATED);
	}
	@GetMapping("view/{id}")
	public ResponseEntity<?> getAdminById(@PathVariable Long id){
			Admin bugEntity=adminService.findAdminById(id);
